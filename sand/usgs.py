...
class DownloadUSGS(BaseDownload):
    ...
    def query(
        ...
        if isinstance(geo, Geo.Point | Geo.Polygon):
            bounds = geo.bounds
            spatial_filter["lowerLeft"] = {
                "latitude": bounds[0],
                "longitude": bounds[1],
            }
            spatial_filter["upperRight"] = {
                "latitude": bounds[2],
                "longitude": bounds[3],
            }
        ...
        r = response.json()
        if r["data"] is None:
            log.error(r["errorMessage"], e=Exception)
        r = r["data"]["results"]
    ...
    def download_file(
        ...
        def _dl(target):
            ...
            if not r["data"]["results"]:
                log.error(
                    f"No product found in collection {self.api_collection} "
                    f"for {product_id}"
                )
            ...
            product = SandProduct(
                product_id=product["displayId"],
                index=product["entityId"],
                date=product["temporalCoverage"]["startDate"],
                metadata=product,
            )
