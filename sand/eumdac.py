...
class DownloadEumDAC(BaseDownload):
    ...
    def query(
        ...
        for collec in self.api_collection:
            ...
            try:
                prod = list(
                    self.selected_collection.search(
                        geo=geo.to_wkt()
        ...
        out = [
            SandProduct(
                product_id=str(d),
                date=d.sensing_start.isoformat(),
                metadata=d,
                index=str(d),
            )
            for d in product
    ...
    def download(
        ...
        data = self.datastore.get_product(
            product_id=product.product_id, collection_id=product.metadata.collection
        )
    ...
    def _download(self, target: Path, data, compression_ext: str | None = None) -> None:
        """
    ...
    def quicklook(self, product: SandProduct, dir: Path | str) -> Path:
        ...
        def _download_qkl(target, url):  # Initialize session for download
            ...
            # Download file
            write(response, target, self.verbose)
