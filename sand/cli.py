...
def entry():
    ...
    parser = ArgumentParser(
        description="SAND search engine CLI tool", epilog="Made by HYGEOS."
    )
    ...
    dl.add_argument(
        "--dir", action="store", help="Directory where to store downloaded product"
    )
    dl.add_argument(
        "product_ids",
    ...
    # > search command
    cmd = subs.add_parser(
        name="search", help="search products in the datasets interfaced by SAND"
    )
    cmd.add_argument(
        "keywords",
        nargs="+",  # Captures ALL remaining arguments
    ...
    cmd.add_argument(
        "--debug", action="store_true", help="Debug mode (developper)", default=False
    )
    cmd.add_argument(
        "--minimum",
        "--min",
        action="store",
        help="Minimum match score to consider [20-100]",
        default=None,
        metavar="match_threshold",
    )

    cmd.add_argument(
        "--from",
        action="store",
        help="Provider selection (Like usgs, cdse, etc..)",
        default=None,
        nargs="+",
        metavar="source",
    )
    ...
    cmd.add_argument(
        "--level",
        action="store",
        help="Level selection (Like L1, L2, etc..)",
        default=None,
        nargs="+",
        metavar="source",
    )
    ...
    mode_group.add_argument(
        "--strict", "-s", action="store_true", help="Strict matching"
    )
    ...
    width_group.add_argument(
        ...
        "-l",
        action="store_true",
        help="Display columns to their max width",
        default=False,
    )
    ...
    cmd.add_argument(
        "--compact", "-c", action="store_true", help="Compact layout", default=False
    )
