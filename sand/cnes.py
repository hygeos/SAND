...
class DownloadCNES(BaseDownload):
    ...
    def query(
        ...
        if isinstance(geo, Geo.Tile):
            if geo.MGRS:
                query["location"] = geo.MGRS
            if geo.venus:
                query["grid:code"] = {"contains": geo.venus}
        ...
        check_too_many_matches(
            response.json(), ["context", "returned"], ["context", "matched"]
        )
        ...
        out = [
            SandProduct(
                product_id=d["properties"]["identifier"],
                index=d["id"],
    ...
    def _download(self, target: Path, url: dict, compression_ext: str | None = None):
        """
        ...
        if compression_ext:
            ...
            log.check(
                _name_difference(target.name, path.name) < 2,
    ...
    def download_file(
        ...
        def _dl(target):
            ...
            prod = SandProduct(
                product_id=r[0]["properties"]["identifier"],
                index=r[0]["id"],
